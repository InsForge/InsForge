...
2. Follow backend conventions.
   ...
   - `deploy/coolify/docker-compose.yml` and `deploy/dokploy/docker-compose.yml` are identical apart from `INSFORGE_DEPLOYMENT_METHOD`, which telemetry reads to tell the two platforms apart. Change both, or one platform silently misses whatever you added.
