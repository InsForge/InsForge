...
function KeySourceBadge({ config }: { config: GatewayConfigResponse }) {
  if (config.keySource === 'byok') {
    return (
      <span className="inline-flex items-center gap-1.5 text-xs font-medium text-emerald-600 bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded-full">
  ...
  if (config.keySource === 'cloud') {
    return (
      <span className="inline-flex items-center gap-1.5 text-xs font-medium text-blue-600 bg-blue-50 border border-blue-200 px-2 py-0.5 rounded-full">
