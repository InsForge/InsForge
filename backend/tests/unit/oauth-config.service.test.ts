...
describe('Google OAuth Provider - extraAuthorizeParams', () => {
  it('merges extraAuthorizeParams into authorization URL', async () => {
    const { GoogleOAuthProvider } = await import(
      '../../src/providers/oauth/google.provider'
    );
    ...
    const url = provider.generateOAuthUrl(config as any, 'test-state');

    expect(url).toContain('prompt=select_account');
    expect(url).toContain('state=test-state');
  });

  it('works without extraAuthorizeParams', async () => {
    const { GoogleOAuthProvider } = await import(
      '../../src/providers/oauth/google.provider'
    );
