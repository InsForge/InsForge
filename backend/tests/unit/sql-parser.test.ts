...
import { parseSQLStatements, checkSystemSchemaOperations, checkAuthSchemaOperations } from '../../src/utils/sql-parser';
...
describe('checkSystemSchemaOperations', () => {
  ...
  it('blocks CREATE TABLE on system schema', () => {
    expect(checkSystemSchemaOperations('CREATE TABLE system.foo (id uuid PRIMARY KEY)')).not.toBeNull();
  });

  it('blocks ALTER TABLE on system schema', () => {
    expect(checkSystemSchemaOperations('ALTER TABLE system.secrets ADD COLUMN foo TEXT')).not.toBeNull();
  });

  it('blocks DELETE on system schema', () => {
    expect(checkSystemSchemaOperations("DELETE FROM system.secrets WHERE key = 'test'")).not.toBeNull();
  });
