...
/**
 * Extract schema name from a libpg-query name list (array of String nodes).
 * Returns the first element if the list has at least one item.
 */
function getSchemaFromNameList(items: Array<Record<string, unknown>>): string | null {
  if (items.length < 1) return null;
  const first = items[0];
 ...
 */
export function checkSystemSchemaOperations(query: string): string | null {
  const isSystem = (s: string | null) => s?.toLowerCase() === 'system';
  ...
  try {
    ...
    for (const stmtWrapper of stmts) {
      ...
      if (stmtType === 'AlterFunctionStmt') {
        const objname =
          ((data.func as Record<string, unknown>)?.objname as Array<Record<string, unknown>>) ?? [];
        if (isSystem(getSchemaFromNameList(objname))) {
          return 'Altering functions in the "system" schema is not allowed.';
        }
      }
