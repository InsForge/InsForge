 ...
 */
computeRouter.post('/', verifyAdmin, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
 ...
 */
computeRouter.get('/:id', verifyAdmin, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
 ...
 * Update a container
 */
computeRouter.patch('/:id', verifyAdmin, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
 ...
 * Delete a container
 */
computeRouter.delete('/:id', verifyAdmin, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
 ...
 */
computeRouter.post('/:id/deploy', verifyAdmin, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    ...
    const deployment = await computeService.deploy({
      container_id: id,
      triggered_by: validation.data.triggered_by,
      github_token: req.body.github_token as string | undefined,
    });

    logger.info('Deployment triggered via API', { containerId: id, deploymentId: deployment.id, actor: req.user?.email });
 ...
 */
computeRouter.get('/:id/deployments', verifyAdmin, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
 ...
 */
computeRouter.get('/:id/deployments/:did', verifyAdmin, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
 ...
 */
computeRouter.post('/:id/rollback/:did', verifyAdmin, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { id, did } = req.params;
    const deployment = await computeService.rollback(id, did);

    logger.info('Rollback triggered via API', { containerId: id, targetDeploymentId: did, newDeploymentId: deployment.id, actor: req.user?.email });
    successResponse(res, deployment, 202);
  } catch (error) {
    next(error);
  }
 ...
 */
computeRouter.get('/:id/logs', verifyAdmin, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
