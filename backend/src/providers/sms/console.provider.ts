 ...
 * is exactly why it fails closed: only explicitly recognized development/test
 * environments may use it — enforced here at send time in addition to the
 * config-save guard.
 */
export class ConsoleSmsProvider implements SmsProvider {
  // async so the environment guard rejects instead of throwing synchronously
