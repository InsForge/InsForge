...
export class VercelProvider {
  ...
  async getDeployment(deploymentId: string): Promise<VercelDeploymentResult> {
    ...
    try {
      ...
      throw new AppError(
        'Failed to get Vercel deployment',
        500,
        errorCodesSchema.enum.INTERNAL_ERROR
      );
    }
  ...
  async cancelDeployment(deploymentId: string): Promise<void> {
    ...
    try {
      ...
      throw new AppError(
        'Failed to cancel Vercel deployment',
        500,
        errorCodesSchema.enum.INTERNAL_ERROR
      );
    }
  ...
  async upsertEnvironmentVariables(envVars: Array<{ key: string; value: string }>): Promise<void> {
    ...
    try {
      ...
      throw new AppError(
        'Failed to upsert environment variables',
        500,
        errorCodesSchema.enum.INTERNAL_ERROR
      );
    }
    ...
    try {
      ...
      if (axios.isAxiosError(error) && error.response?.status === 404) {
        throw new AppError(
          ...
          404,
          errorCodesSchema.enum.ENVIRONMENT_VARIABLE_NOT_FOUND
        );
      }
  ...
  async getCustomDomainConfig(domain: string): Promise<VercelDomainConfig> {
    ...
    try {
      ...
      throw new AppError(
        'Failed to get custom domain config',
        500,
        errorCodesSchema.enum.INTERNAL_ERROR
      );
    }
    ...
    try {
      ...
      if (axios.isAxiosError(error) && error.response?.status === 404) {
        throw new AppError(
          ...
          404,
          errorCodesSchema.enum.DOMAIN_NOT_FOUND
        );
      }
      ...
      throw new AppError('Failed to get custom domain', 500, errorCodesSchema.enum.INTERNAL_ERROR);
    }
  }
  ...
  async addCustomDomain(domain: string): Promise<{
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        ...
        if (status === 409) {
          throw new AppError(
            ...
            409,
            errorCodesSchema.enum.DOMAIN_ALREADY_EXISTS
          );
        }
        if (status === 400) {
          throw new AppError(
            ...
            400,
            errorCodesSchema.enum.DOMAIN_INVALID
          );
        }
      }
      ...
      throw new AppError('Failed to add custom domain', 500, errorCodesSchema.enum.INTERNAL_ERROR);
    }
  }
  ...
  async uploadFile(fileContent: Buffer): Promise<string> {
    ...
    for (let attempt = 0; attempt <= UPLOAD_MAX_RETRIES; attempt++) {
      try {
        ...
        throw new AppError(
          'Failed to upload file to Vercel',
          500,
          errorCodesSchema.enum.INTERNAL_ERROR
        );
      }
    ...
    try {
      ...
      if (axios.isAxiosError(error) && error.code === 'ERR_CANCELED') {
        throw new AppError(
          ...
          499,
          errorCodesSchema.enum.UNKNOWN_ERROR
        );
      }
