...
export const storeConversationRequestSchema = z.object({
  ...
  metadata: z.record(z.unknown()).optional(),
  messages: z.array(messageInputSchema).min(1, 'At least one message is required'),
  embeddingModel: z
    .string()
    .optional()
...
export const searchConversationsRequestSchema = z.object({
  ...
  embeddingModel: z.string().optional().describe('Embedding model to use for the search query'),
});
