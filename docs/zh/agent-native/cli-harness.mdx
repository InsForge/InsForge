...
| 项目 | `create`, `link`, `projects update`, `projects update-version`, `projects delete` |
...
`projects update-version` 将项目迁移到最新的 InsForge 后端版本。默认针对已链接的项目；传递 `--project <id>` 以选择其他项目。该命令会重启实例，因此预期会有短暂停机。添加 `--wait` 以阻塞直到更新完成。
...
`projects update` 就地更改项目设置。传入 `--name "<新名称>"` 可重命名项目；名称至少为 2 个字符，且在您的组织内必须唯一。它默认针对已链接的项目，因此传入 `--project <id>` 可重命名其他项目。该命令在底层会发出 `PUT /projects/v1/{projectId}` 请求，请求体为 `{ "name": "<新名称>" }`。

您也可以在仪表板的 **Settings → General** 下重命名项目。
