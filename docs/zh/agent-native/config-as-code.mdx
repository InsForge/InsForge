---
title: 代码配置
description: "通过 insforge.toml 声明式文件对 InsForge 配置进行版本控制：涵盖身份验证策略、重定向 URL、密码规则、SMTP、存储上传大小与部署子域，配合 CLI 的 plan、apply、export 三个命令实现代码化审查与应用配置变更，让每次变更都可审计回溯。"
---
