---
title: "pgvector：Postgres 向量搜索"
description: "pgvector 扩展在每个 InsForge 项目的 Postgres 数据库上开箱可用：直接存储嵌入向量并运行相似性搜索，为语义搜索、推荐系统与检索增强生成（RAG）提供原生支持，页面含提示编码代理创建向量表、写入嵌入与执行相似度查询的示例，无需部署独立向量数据库。"
---
