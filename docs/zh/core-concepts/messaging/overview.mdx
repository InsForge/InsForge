...
description: "通过 InsForge 托管发件服务或自定义 SMTP 从项目发送事务性电子邮件；手机登录验证码现已通过短信发送，推送通知在路线图上并将共用同一发送 API。"
---

InsForge Messaging 从您的项目发送事务性通知：收据、摘要、密码重置代码、通知汇总、任何您本来会为其接入 SendGrid、Postmark 或 Twilio 的东西。电子邮件是第一个渠道。短信已可用于手机登录验证码（参见[自定义短信](/core-concepts/messaging/custom-sms)）；通用短信发送 API 和推送在路线图上，将共享相同的 API 表面。
