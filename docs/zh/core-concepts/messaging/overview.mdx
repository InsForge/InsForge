...
<Note>
  **按需邮件需要付费计划。** 使用 `emails.send()` 发送事务性消息仅在付费计划中可用——在 **Free** 计划上，该调用会返回 `On-demand email service is only available for paid plans`，且不会发送任何邮件。身份验证邮件（验证码、密码重置、一次性验证码）是例外：它们在所有计划上都会发送，仅受每月邮件用量限制。

  **每个收件人单独计数。** `to`、`cc` 和 `bcc` 中的每个地址都会作为一次发送计入速率限制和每月用量——一封发送给三个收件人的邮件即为三次发送。付费计划采用固定的一小时速率限制窗口；每小时的具体额度取决于您的计划。使用 `npx @insforge/cli usage` 查看当前计划，并参阅 [定价](/pricing) 了解各计划包含的内容。
</Note>
