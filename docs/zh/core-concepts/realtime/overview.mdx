...
description: "客户端订阅 order:123、chat:room-1 等 InsForge 实时频道，经 WebSocket（Socket.IO）接收数据库变更、广播消息与在线状态（presence）更新；频道还能把相同事件分发到 Webhook URL，让应用与其他服务无需刷新页面即可低延迟联动。"
---
