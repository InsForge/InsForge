...
<Warning>
  删除桶是永久性操作。它会级联删除桶中的每一个对象，并且没有回收站、软删除或恢复机制——无论是自助方式还是通过技术支持都无法恢复。请先下载所有需要的文件，仅对确定不再需要的桶执行删除操作（`DELETE /api/storage/buckets/{bucketName}`、`delete-bucket` MCP 工具或 SDK）。
</Warning>
