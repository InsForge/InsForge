...
<Tabs>
  <Tab title="Docker（你自己的宿主机）">
    ...
    启用方式是一个明确的动作：把 Docker socket 挂载进 InsForge 容器。compose 文件里已经为此准备好了两行，取消注释即可：
    ...
    `group_add` 是必需的：socket 的权限是 `660 root:docker`，没有匹配的组，后端会拿到 `EACCES`。这个 group id 因宿主机而异，请把你的宿主机报告的值填到 `.env` 的 `DOCKER_GID` 一行：

    <CodeGroup>
    ```bash Linux
    # Amazon Linux 2023 上是 993，Debian/Ubuntu 上常见为 999
    getent group docker | cut -d: -f3
    ```

    ```bash Docker Desktop（macOS/Windows）
    # VM 内的 socket 属主是 root:root，所以用 0
    echo 0
    ```
    </CodeGroup>
  ...
  <Tab title="Fly.io">
    ...
    - `FLY_API_TOKEN`：Fly.io API token，用 `fly tokens create org` 生成。InsForge 用它来创建和管理 compute 容器。
...
每个服务选择一种 ingress 模式。默认是 `none`，因为大多数 compute——队列 worker、处理器、推理循环——根本不接收入站流量。
