...
description: "使用 InsForge Custom Compute 在项目旁运行长期存活的容器：队列 worker、后台处理器、AI 推理循环、websocket 服务与爬虫，支持容器部署、资源配置、日志、Secrets 与环境变量管理，并涵盖自托管实例启用 compute 的完整步骤。"
---
...
对于现成的服务——比如 Redis 缓存、nginx 代理——把服务指向一个公共镜像即可拉取并启动，不需要 `Dockerfile` 或构建步骤。使用 CLI：
...
`--image` 选择镜像模式（不需要 `flyctl` 或 Docker）。像 Redis 这样的裸 TCP 服务请用 `--protocol tcp`；默认的 `--protocol http` 适用于 HTTP 服务器。用相同的 `--name` 再次运行 `compute deploy` 会就地更新已有服务。

由于 Custom Compute 暂不支持持久化卷，请把 Redis 容器当作临时缓存——一旦镜像、环境变量或端口发生变更，其中保存的内容都会丢失。需要持久保留的数据请放进项目的 Postgres 或 Storage。
