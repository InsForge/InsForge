...
Dashboard 和 CLI 都调用同一个管理端点。你也可以用管理 API Key（`ik_...`）直接调用它：
...
- `PUT /api/payments/stripe/{environment}/config`，其中 `{environment}` 为 `test` 或 `live`。仅限管理员。
- 请求体为 `{ "secretKey": "..." }`。密钥前缀必须与环境匹配（`test` 用 `sk_test_...`，`live` 用 `sk_live_...`）。
- 成功后，InsForge 会对照你的 Stripe 账户验证密钥、加密存储到秘密存储、尝试（重新）创建托管的 webhook 端点，并为 Products、Prices、Customers 和 Subscriptions 运行首次同步。如果连接的 Stripe 账户发生变化，会先清除之前镜像的支付数据。

要读取连接状态和掩码后的密钥，调用 `GET /api/payments/stripe/status`。
