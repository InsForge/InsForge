---
title: 存储API参考
description: 通过REST API的文件存储和桶管理
---
...
| 字段 | 类型 | 必需 | 说明 |
|-------|------|----------|-------------|
| `filename` | string | 是 | 对象键；除非启用 `autoKey`，否则按原样使用 |
| `contentType` | string | 否 | 文件的MIME类型 |
| `size` | integer | 否 | 文件大小（字节） |
| `upsert` | boolean | 否 | 替换该键的现有对象。默认为 `false` |
| `autoKey` | boolean | 否 | 根据 `filename` 生成唯一键。默认为 `false` |
...
确认文件已成功使用预签名 URL 上传到 S3。请使用上传策略返回的完整 `confirmUrl`；对于 S3，该 URL 包含由服务器管理的暂存标识符。
...
| `upsert` | boolean | 否 | 必须与请求上传策略时使用的 `upsert` 值一致 |
