---
title: "TypeScript 身份验证 SDK 参考"
description: "InsForge TypeScript 身份验证 SDK 参考：使用 signUp、signInWithPassword 与 signInWithOAuth 实现注册、密码登录与 OAuth 第三方登录，涵盖邮箱验证、密码重置、会话管理与用户资料读写，覆盖 Node、浏览器与边缘运行时。"
---
