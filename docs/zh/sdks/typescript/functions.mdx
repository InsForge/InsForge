---
title: "TypeScript 函数 SDK 参考"
description: "InsForge TypeScript 函数 SDK 参考：使用 invoke 从 Node、浏览器与边缘运行时调用无服务器边缘函数，传入 JSON 请求体、自定义标头与查询参数并处理响应与错误，提供从编写函数到客户端调用的完整端到端示例，便于集成到应用业务逻辑中。"
---
