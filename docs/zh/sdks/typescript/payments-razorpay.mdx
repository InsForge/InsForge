---
title: "Razorpay 支付 SDK（TypeScript）"
description: "InsForge Razorpay 支付 SDK 参考（TypeScript）：加载 Razorpay Checkout、创建一次性订单与订阅、管理与取消订阅、通过 Webhook 完成支付履行并将数据同步到 Postgres，涵盖 SDK 设置与生产测试环境切换。"
---
