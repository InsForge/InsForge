---
title: "Stripe 支付 SDK（TypeScript）"
description: "InsForge Stripe 支付 SDK 参考（TypeScript）：使用 createCheckoutSession 与 createCustomerPortalSession 创建结账与账单门户会话，读取付款状态并将订阅写入 Postgres，涵盖授权、RLS 与测试环境。"
---
