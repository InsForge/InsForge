---
title: "TypeScript 数据库 SDK 参考"
description: "InsForge TypeScript 数据库 SDK 参考：对 Postgres 表执行类型安全的 insert、update、delete、select 与 rpc 调用，支持 PostgREST 风格筛选器、修饰符与自动生成类型，并涵盖分页排序等常见模式。"
---
