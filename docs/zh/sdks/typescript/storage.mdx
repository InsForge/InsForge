---
title: 存储 SDK 参考
description: 使用 InsForge TypeScript SDK 进行文件上传、下载和管理
---
...
- `options.upsert` (boolean, 可选) - 替换现有对象。默认为 `false`
...
<Warning>
如果已存在相同密钥的文件，`upload()` 默认返回冲突。传入 `{ upsert: true }` 可替换该文件；如需服务器生成唯一密钥，请使用 `uploadAuto()`。
</Warning>
