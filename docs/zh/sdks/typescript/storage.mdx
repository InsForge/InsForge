---
title: "TypeScript 存储 SDK 参考"
description: "InsForge TypeScript 存储 SDK 参考：从 Node、浏览器与边缘运行时使用 upload、uploadAuto、download 与 remove 管理文件，通过 from 选择存储桶并处理上传与下载策略，用于安全上传、共享与管理应用资源。"
---
