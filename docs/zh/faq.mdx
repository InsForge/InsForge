...
<AccordionGroup>
  ...
  <Accordion title="我可以自己手动暂停项目吗？">
    可以。手动暂停和上面说的闲置自动暂停是两回事，而且不限于 Free 计划——organization 里任何拥有 **Developer** 或 **Administrator** 角色的成员，都可以主动暂停一个活跃项目来释放资源。暂停期间你的数据完好无损。CLI 没有 `pause` 命令；请在 dashboard 里暂停，或在 InsForge Cloud 上直接调用平台 API：
    ...
    想让项目重新上线，就恢复它——参见下面的“我的项目被暂停了，怎么重新启动？”（在 dashboard 里点 **Restore Project**，或运行 `npx @insforge/cli projects restore --project <id>`）。
  </Accordion>
