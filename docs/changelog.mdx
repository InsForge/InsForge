...
<Update label="Mar 24, 2026" tags={["Auth"]}>
  ## Headless Device Authorization

  Added the first release of the instance-scoped headless device authorization flow for non-browser clients. It reuses the existing sign-in and consent pages, and the auth docs now cover the create, lookup, approve/deny, and token exchange endpoints.

  [View Authentication architecture →](/core-concepts/authentication/architecture)
</Update>
