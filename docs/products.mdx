...
InsForge ships seven core products and three private-preview modules. Every
one is exposed through the same REST surface, the same JWT auth, the same
MCP server, and the same SDKs.
...
<CardGroup cols={2}>
  ...
  <Card title="Deployments" icon="rocket" href="/core-concepts/deployments/architecture">
...
These modules are functional but the API may change before general availability.
...
<CardGroup cols={3}>
  ...
  <Card title="Payments" icon="credit-card" href="/core-concepts/payments/architecture">
    Stripe checkout, subscriptions, and customer portal flows.
  </Card>

  <Card title="Compute" icon="server" href="/core-concepts/compute/architecture">
