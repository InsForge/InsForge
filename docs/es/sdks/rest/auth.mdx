...
description: Autenticación de usuario y gestión de sesiones a través de API REST
---
...
```
POST /api/auth/email/send-sign-in-otp
```
...
```bash
curl -X POST "https://your-app.insforge.app/api/auth/email/send-sign-in-otp" \
  -H "Content-Type: application/json" \
...
```bash
curl -X POST "https://your-app.insforge.app/api/auth/sessions/otp?client_type=server" \
  -H "Content-Type: application/json" \
  -d '{
    "email": "user@example.com",
    "otp": "123456",
