...
description: Almacenamiento de archivos y gestión de cubos a través de API REST
---
...
|-------|------|----------|-------------|
| `filename` | string | Sí | Clave de objeto usada literalmente, salvo que se active `autoKey` |
| `contentType` | string | No | Tipo MIME del archivo |
| `size` | integer | No | Tamaño de archivo en bytes |
| `upsert` | boolean | No | Reemplaza un objeto existente con esta clave. Por defecto es `false` |
| `autoKey` | boolean | No | Genera una clave única a partir de `filename`. Por defecto es `false` |
...
```json
{
  ...
  "fields": {
    "bucket": "my-s3-bucket",
    "key": "app-key/_uploads/11111111-2222-4333-8444-555555555555",
  ...
  "confirmUrl": "/api/storage/buckets/avatars/objects/profile-photo.jpg/confirm-upload?uploadId=11111111-2222-4333-8444-555555555555",
...
Confirma que un archivo se ha cargado correctamente a S3 usando una URL presignada. Usa exactamente la `confirmUrl` devuelta por la estrategia; para S3 contiene el identificador de preparación administrado por el servidor.
...
| `upsert` | boolean | No | Debe coincidir con el valor de `upsert` usado al solicitar la estrategia |

#### Ejemplo

```bash
curl -X POST "https://your-app.insforge.app/api/storage/buckets/avatars/objects/profile-photo.jpg/confirm-upload?uploadId=11111111-2222-4333-8444-555555555555" \
  -H "Authorization: Bearer your-jwt-token" \
  -H "Content-Type: application/json" \
  -d '{
    "size": 102400,
