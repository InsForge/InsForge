...
description: Carga, descarga y gestión de archivos con el SDK de TypeScript para InsForge
---
...
- `options.upsert` (boolean, opcional) - Reemplaza un objeto existente. Por defecto es `false`
...
<Warning>
Si ya existe un archivo con la misma clave, `upload()` devuelve un conflicto por defecto. Usa `{ upsert: true }` para reemplazarlo o `uploadAuto()` para que el servidor genere una clave única.
</Warning>
