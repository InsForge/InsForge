...
<AccordionGroup>
  ...
  <Accordion title="¿Cómo permito que los usuarios inicien sesión con GitHub (u otro proveedor OAuth)?">
    Iniciar sesión con GitHub consta de dos partes: habilita GitHub como proveedor una vez y luego arranca el flujo desde tu aplicación con el SDK.

    **1. Habilita GitHub.** En el panel, abre **Auth Methods** y habilita GitHub. En InsForge Cloud puedes usar la app de GitHub compartida de InsForge, así que no hay credenciales que rellenar. Para usar tu propia app OAuth de GitHub en su lugar (obligatorio al autoalojar), crea una en GitHub bajo **Settings → Developer settings → OAuth Apps**, pega su **Client ID** y su **Client Secret**, y establece la **Authorization callback URL** de la app en:

    ```text
    <la-url-de-tu-proyecto>/api/auth/oauth/github/callback
    ```

    GitHub es uno de los proveedores integrados (`google`, `github`, `discord`, `linkedin`, `facebook`, `instagram`, `tiktok`, `apple`, `x`, `spotify`, `microsoft`); los pasos son los mismos para los demás. Cuando usas tu propia app, el scope de GitHub solicitado por defecto es `user:email`.

    **2. Permite la URL de redirección de tu aplicación.** El `redirectTo` que pasas abajo debe estar en las URLs de redirección permitidas de tu proyecto (configuradas en la configuración de autenticación); de lo contrario, el inicio de sesión se rechaza.

    **3. Arranca el flujo desde tu aplicación** con `signInWithOAuth()`, pasando `github` como proveedor:
    ...
    El navegador redirige a GitHub y, tras la aprobación del usuario, InsForge lo devuelve a `redirectTo` con un parámetro de consulta `insforge_code`. El SDK detecta ese código automáticamente, lo intercambia por una sesión y la guarda. Consulta [`signInWithOAuth()`](/sdks/typescript/auth#signinwithoauth) para la referencia completa —incluido `skipBrowserRedirect` para flujos renderizados en el servidor o móviles— y la [Descripción general de Autenticación](/core-concepts/authentication/overview).
  </Accordion>
</AccordionGroup>
