...
<AccordionGroup>
  ...
  <Accordion title="¿Puedo poner mi API de backend en un dominio propio (como api.example.com)?">
    Sí. Esto es distinto de un dominio personalizado de [Sites](/core-concepts/sites/overview), que apunta a un frontend desplegado. Aquí vuelves a apuntar la **API de backend** que InsForge sirve por defecto en `https://<appkey>.<region>.insforge.app` (por ejemplo `https://<appkey>.us-east.insforge.app`) a un dominio que tú controlas.

    Configura el dominio personalizado desde el CLI:

    ```bash
    npx @insforge/cli projects update --domain api.example.com
    ```

    Esto apunta al proyecto vinculado por defecto; pasa `--project <id>` para actualizar otro. Por debajo llama a `PUT /projects/v1/{projectId}` con un campo `customizedDomain`, así que el mismo cambio está disponible a través de la admin API. Solo un **Administrator** de la organización puede cambiar el dominio de un proyecto. Una vez configurado, la capa de enrutamiento de InsForge sirve tu API de backend en ese hostname junto con la URL `insforge.app` por defecto.

    Aún tienes que apuntar el dominio a InsForge con un registro DNS en tu registrador, tras lo cual InsForge termina el TLS para ese hostname. {{TODO: confirm dashboard location}} — el registro DNS exacto (el destino del CNAME) y el paso de verificación/provisión de TLS para un dominio de backend no están en la superficie del CLI/API, así que hasta que se confirmen, pídele al equipo de InsForge en [Discord](https://discord.com/invite/DvBtaEc9Jz) el registro que debes crear.
  </Accordion>
</AccordionGroup>
