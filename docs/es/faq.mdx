...
<AccordionGroup>
  ...
  <Accordion title="¿Cómo importo o cargo una base de datos Postgres existente en InsForge?">
    ...
    pg_restore --no-owner -d "postgresql://postgres:<password>@<appkey>.<region>.database.insforge.app:5432/insforge?sslmode=require" local.dump

    # ...o un archivo SQL plano
    psql "postgresql://postgres:<password>@<appkey>.<region>.database.insforge.app:5432/insforge?sslmode=require" < local.sql
    ```
    ...
    - Una vez importada, cada tabla queda expuesta como un endpoint REST y de SDK, igual que cualquier otra tabla de InsForge. Si prefieres versionar los cambios de esquema en git, consulta [Migraciones de base de datos](/core-concepts/database/migrations).
  </Accordion>
