...
<Warning>
  Eliminar un depósito es permanente. Elimina en cascada todos los objetos que
  contiene, y no hay papelera, borrado temporal ni restauración —ni por cuenta
  propia ni a través de soporte técnico—. Descargue primero lo que necesite y
  reserve la acción de eliminar (`DELETE /api/storage/buckets/{bucketName}`, la
  herramienta MCP `delete-bucket` o el SDK) para depósitos que esté seguro de no
  volver a necesitar.
</Warning>
