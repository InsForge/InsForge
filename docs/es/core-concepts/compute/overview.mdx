...
<Tabs>
  <Tab title="Docker (tu propio host)">
    ...
    Activarlo es un acto deliberado: monta el socket de Docker en el contenedor de InsForge. En tu archivo compose, descomenta las dos líneas que ya están ahí para esto:
    ...
    `group_add` es obligatorio porque el socket tiene modo `660 root:docker`: sin un grupo que coincida, el backend recibe `EACCES`. El id de grupo depende del host, así que pon el valor que informe tu host en la línea `DOCKER_GID` de `.env`:

    <CodeGroup>
    ```bash Linux
    # 993 en Amazon Linux 2023, habitualmente 999 en Debian/Ubuntu
    getent group docker | cut -d: -f3
    ```

    ```bash Docker Desktop (macOS/Windows)
    # El socket dentro de la VM pertenece a root:root, así que usa 0
    echo 0
    ```
    </CodeGroup>
  ...
  <Tab title="Fly.io">
    ...
    - `FLY_API_TOKEN`: un token de la API de Fly.io, creado con `fly tokens create org`. InsForge lo usa para crear y gestionar tus contenedores de compute.
...
Cada servicio elige un modo de ingress. El valor por defecto es `none`, porque la mayor parte del compute —workers de cola, procesadores, bucles de inferencia— no recibe tráfico entrante en absoluto.
...
```bash
# 1. Reserva el nombre (todavía sin imagen)
