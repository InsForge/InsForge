...
Para un servicio estándar —una caché Redis, un proxy nginx— apunta un servicio a una imagen pública y se descarga y arranca, sin `Dockerfile` ni paso de construcción. Desde la CLI:
...
`--image` selecciona el modo imagen (sin necesidad de `flyctl` ni Docker). Usa `--protocol tcp` para servicios de TCP en crudo como Redis; el `--protocol http` por defecto sirve para servidores HTTP. Volver a ejecutar `compute deploy` con el mismo `--name` actualiza el servicio existente en el sitio.

Como Custom Compute todavía no tiene volúmenes persistentes, trata un contenedor de Redis como una caché efímera: todo lo que contenga se descarta cuando cambian la imagen, las variables de entorno o el puerto. Guarda los datos duraderos en el Postgres o el Storage de tu proyecto.
