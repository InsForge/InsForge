...
- **One-shot tasks via ECS `RunTask`** (see below)

### Phase 2: One-Shot Tasks (ECS RunTask)

Phase 1 uses `CreateService` — containers run forever, serve HTTP, attached to ALB. Phase 2 adds `RunTask` — containers run once, exit when done, no ALB.

**Use cases:**
- Batch jobs (data processing, ETL)
- Test runners (Playwright, Cypress, etc.)
- Database migrations
- One-off scripts (backfill, cleanup)
- Scheduled cron tasks

**How it works:**

```
ECS CreateService (Phase 1)     ECS RunTask (Phase 2)
─────────────────────────       ──────────────────────
Always running                  Runs once, exits
Attached to ALB                 No ALB (no HTTP endpoint)
Auto-restart on crash           No restart
Pay 24/7                        Pay per execution
$30/month                       ~$0.01/run

Trigger: HTTP request           Trigger: API call, cron, webhook, DB event
```
...
| Trigger | How |
|---|---|
| Manual | Dashboard button "Run Task" or `POST /compute/containers/:id/run` |
| Cron | InsForge schedules feature triggers `RunTask` on interval |
| Webhook | Incoming webhook triggers `RunTask`, returns task ID, caller polls for result |
| DB event | Postgres `NOTIFY` → backend listener → `RunTask` |

**API additions:**

| Method | Route | Description |
|---|---|---|
| `POST` | `/compute/containers/:id/run` | Run container as one-shot task |
| `GET` | `/compute/tasks/:taskId` | Get task status + exit code |
| `GET` | `/compute/tasks/:taskId/logs` | Get task output logs |

**Data model addition:**

```sql
CREATE TABLE IF NOT EXISTS compute.tasks (
  ...
  triggered_by    TEXT DEFAULT 'manual'
                  CHECK (triggered_by IN ('manual', 'cron', 'webhook', 'db_event'))
);
```
...
run_mode TEXT NOT NULL DEFAULT 'service'
  CHECK (run_mode IN ('service', 'task'));
```

Containers with `run_mode = 'service'` work as Phase 1 (CreateService, ALB, always-on). Containers with `run_mode = 'task'` use RunTask — no ALB, no always-on, invoked on demand.
...
interface ComputeProvider {
  ...
  getTaskLogs(taskArn: string, opts: LogOpts): Promise<LogStream>;
  stopTask(taskArn: string): Promise<void>;
}
```

**Implementation:**
- Same task definition, same ECR image, same VPC/subnets — reuses all Phase 1 infrastructure
- `RunTask` instead of `CreateService` — single API call difference
- No ALB target group or listener rule needed
- Task logs go to same CloudWatch log group
- Exit code captured from ECS task metadata

**Dashboard UI:**

For `run_mode = 'task'` containers, the detail view changes:
- No "endpoint URL" (there is none)
- "Run Task" button instead of "Redeploy"
- "Tasks" tab instead of "Deployments" — shows run history with exit codes
- Status shows last run result, not always-on status

**Cost comparison:**

| Scenario | CreateService | RunTask |
|---|---|---|
| Test runner, 10 runs/day, 5 min each | $30/month (always on) | ~$1.50/month |
| Daily ETL, 15 min | $30/month | ~$0.45/month |
| Hourly cron, 1 min each | $30/month | ~$1.35/month |

RunTask is dramatically cheaper for workloads that don't need to be always-on.
