...
> - **D Test CLI prompt** now uses a real `uak_…` user API key minted by the cloud control plane on every CLI tab mount (`onRequestUserApiKey` host callback → postMessage `REQUEST_USER_API_KEY`); falls back to `<placeholder>` only when the host doesn't provide the callback (self-hosted preview). Includes `--template todo` to scaffold the demo project.
