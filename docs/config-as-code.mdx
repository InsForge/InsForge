...
`insforge.toml` is a declarative, version-controlled snapshot of a subset of your project's config — auth policy, allowed redirect URLs, password rules, SMTP, and deployment subdomain. The CLI provides three commands that work against this file:
...
All three commands accept `--file <path>` / `--out <path>` if your TOML lives somewhere other than `./insforge.toml`.
...
1. **Per-change capability gate.** Each change is checked against the backend's metadata. If the backend doesn't support a section (e.g. an older self-hosted instance without SMTP exposed), that section is skipped with a named warning — the rest of the changes still apply.
...
3. **Per-section dispatch.** Each change is sent to the appropriate backend endpoint (`/api/auth/config`, `/api/auth/smtp-config`, `/api/deployments/slug`, etc.). Changes are independent — a failure on one section won't roll back earlier successful sections.
...
The file written by `export` is the same shape the CLI expects from `apply` — round-tripping is supported.
...
`env(...)` refs in a TOML that's the target of `apply` re-send the resolved password on every run — that's the only way the CLI can tell the backend "the secret may have rotated, please update." Fields without an `env(...)` ref are treated as preserve-existing.

## Self-Hosting Use Cases

Config-as-code is especially valuable on self-hosted InsForge deployments:

- **Version control for auth.** Redirect URLs, password policy, email verification mode — all the things you'd otherwise change through the dashboard or curl — live in a file your team reviews via PR.
...
- **Disaster recovery.** A committed `insforge.toml` is a known-good config snapshot. Re-apply it after restoring a backend to bring auth/SMTP back to the expected shape in seconds.
- **Local OSS development.** Run `insforge link --api-base-url http://localhost:7130 --api-key <local-key>` against a docker-compose stack, then point the CLI's config commands at your local OSS instance the same way you'd point at cloud.
...
- [CLI overview](https://github.com/InsForge/CLI) — full command reference and global flags
- [Self-Hosting Deployment Security Guide](/deployment/deployment-security-guide) — hardening a self-hosted backend after deploy
