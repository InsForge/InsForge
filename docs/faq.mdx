...
<AccordionGroup>
  ...
  <Accordion title="Can I pause a project myself?">
    Yes. Manual pausing is separate from the automatic inactivity pause described above, and it isn't limited to the Free plan — any organization member with the **Developer** or **Administrator** role can pause an active project on purpose to free up its resources. Your data stays intact while the project is paused. There is no CLI `pause` command; pause it from the dashboard, or, on InsForge Cloud, call the platform API directly:
    ...
    To bring the project back online, restore it — see "My project is paused. How do I restart it?" below (click **Restore Project** in the dashboard, or run `npx @insforge/cli projects restore --project <id>`).
  </Accordion>
