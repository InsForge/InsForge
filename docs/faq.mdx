...
<AccordionGroup>
  ...
  <Accordion title="Can I put my backend API on my own custom domain (like `api.example.com`)?">
    Yes. On InsForge Cloud a project's backend API (the default `https://<appkey>.<region>.insforge.app` URL) can be served from a custom domain. This is separate from a [Sites](/core-concepts/sites/overview) custom domain, which is for your deployed frontend.

    Set the domain with the CLI. This requires the organization **Administrator** role:

    ```bash
    npx @insforge/cli projects update --project <id> --domain api.example.com
    ```

    Or call the Cloud admin API directly:
    ...
    The domain must be at least 4 characters. Once it's set, InsForge returns your custom domain as the project's canonical URL (`https://api.example.com`). Confirm it landed with:

    ```bash
    npx @insforge/cli projects get --project <id>
    ```

    which prints a `Domain:` line when a custom domain is configured.

    Setting the domain on the project is not the whole story: you still need a DNS record at your provider that points the hostname at InsForge, and a TLS certificate is provisioned for it. If the domain doesn't resolve or the certificate hasn't come up after you set it, ask the InsForge team in [Discord](https://discord.com/invite/DvBtaEc9Jz).
    ...
    Self-hosting InsForge instead? You control the reverse proxy and TLS yourself — see the [self-hosting guides](/deployment/deploy-to-aws-ec2).
  </Accordion>
</AccordionGroup>
