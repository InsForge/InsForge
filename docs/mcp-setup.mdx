...
description: "Connect Cursor, Claude, Windsurf, and other editors to InsForge over MCP so AI agents can query your database, auth, storage, and functions."
---
...
The fastest way to get started is with Remote MCP. A single command attempts installation and tries to handle authentication and project binding. You may still be prompted for manual steps if the automatic flow cannot complete.
...
Some MCP clients automatically prompt you to log in during setup. Others may require manual authentication or project binding steps if the automatic flow cannot complete. Either way, authentication opens a browser window where you can log in to your InsForge account and grant organization/project access to the MCP client.
...
<AccordionGroup>
  <Accordion title="Cursor">
    ...
    If the InsForge MCP server shows the status "Needs authentication", click the `connect` button to authenticate.
  </Accordion>
