...
If you use the InsForge CLI and have already run `npx @insforge/cli link` to link your project, you can connect or disconnect any MCP provider with a single command — no manual config editing required.
...
This writes the InsForge MCP server entry directly into the provider's local config file (e.g. `.cursor/mcp.json`) using your linked project's API key and host URL, then marks the dashboard as **Connected**.
...
This removes only the `insforge` entry from the provider's config file and marks the dashboard as **Disconnected**. All other MCP servers remain untouched.
