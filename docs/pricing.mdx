...
<AccordionGroup>
  ...
  <Accordion title="How do I check my current usage, and when do the limits reset?">
    ...
    (database, file storage, bandwidth/egress, AI credits, edge function calls,
    and email requests), followed by a per-project breakdown of database,
    storage, and egress.

    Consumption counters (bandwidth/egress, AI credits, edge function calls, and
    emails) are measured per billing period and start again from zero at the
