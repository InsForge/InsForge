...
Connect PostHog once with one click, drop the setup prompt into your coding agent so it instruments `@insforge/sdk` events on your frontend, and the Analytics pages start filling in.
...
`@insforge/cli posthog setup` links your InsForge project to PostHog, then hands off to the official [PostHog wizard](https://posthog.com/docs/libraries/wizard) (`npx -y @posthog/wizard@latest`) to wire the SDK into your code. The wizard detects your framework, installs the right PostHog SDK, and drops in initialization code so pageviews, autocapture events, and session recordings start flowing.
