...
### Set the secret key via the admin API

The dashboard and CLI both call the same admin endpoint. You can also call it directly with an admin API Key (`ik_...`):
...
- `PUT /api/payments/stripe/{environment}/config`, where `{environment}` is `test` or `live`. Admin only.
- The request body is `{ "secretKey": "..." }`. The key prefix must match the environment (`sk_test_...` for `test`, `sk_live_...` for `live`).
- On success InsForge validates the key against your Stripe account, stores it encrypted in the secret store, tries to (re)create the managed webhook endpoint, and runs the initial sync for Products, Prices, Customers, and Subscriptions. If the connected Stripe account changed, previously mirrored payment data is cleared first.

To read the connection status and a masked key, call `GET /api/payments/stripe/status`.
