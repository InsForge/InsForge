...
Subscription creation first checks the caller against `payments.razorpay_subscriptions` RLS `INSERT` policies, so your app can decide who can start a subscription for a team, workspace, or organization. Cancel, pause, and resume check the same table's `UPDATE` policies through a rollbacked `updated_at` probe.
