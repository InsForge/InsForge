...
Razorpay uses Items and Plans instead of Stripe Products and Prices. An Item is the amount-bearing unit being sold. A Plan is a recurring subscription definition built around an item. Razorpay catalog mutations call Razorpay first, then sync the result into InsForge.
...
Subscription creation first checks the caller against `payments.razorpay_subscriptions` RLS `INSERT` policies, so your app can decide who can start a subscription for a team, workspace, or organization. Cancel, pause, and resume check the same table's `UPDATE` policies.
