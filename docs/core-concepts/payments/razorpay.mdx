...
InsForge copies request `metadata` to Razorpay `notes`. If your fulfillment trigger reads `notes.order_id`, pass `metadata: { order_id: ... }` when creating the Order or Subscription.
