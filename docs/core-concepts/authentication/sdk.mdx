...
<Note>
  When `requireEmailVerification` is true, `accessToken` will be null until the user verifies their email.
</Note>
...
Get authenticated user with profile data from users table. Returns auth info and profile as separate objects.
...
```typescript
{
  data: {
    ...
    profile: { id, name, avatar_url, createdAt, updatedAt, ...customFields } | null
  } | null,
  error: Error | null
}
```
...
if (data) {
  ...
  console.log(data.user.role); // 'authenticated' or 'project_admin'
  console.log(data.profile?.name); // Profile from users table
}
```
...
Get current session from local storage (no API call). This is a synchronous method.
...
```javascript
const { data, error } = insforge.auth.getCurrentSession();
...
Get all public authentication configuration including OAuth providers and email auth settings. This is a public endpoint that doesn't require authentication.
...
if (data) {
  console.log(`OAuth providers: ${data.oAuthProviders.length}`);
  console.log(`Password min length: ${data.passwordMinLength}`);
  console.log(`Email verification required: ${data.requireEmailVerification}`);
}
```
...
Send email verification using the method configured in auth settings (`verifyEmailMethod`). When method is `code`, sends a 6-digit numeric code. When method is `link`, sends a magic link.
...
```javascript
const { data, error } = await insforge.auth.sendVerificationEmail({
  email: 'user@example.com',
});
