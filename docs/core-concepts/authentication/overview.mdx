...
<Note>
  **Self-hosting?** Sending these emails requires an email provider. Cloud-linked projects use the managed sender automatically; a self-hosted instance must set up [Custom SMTP](/core-concepts/messaging/custom-smtp) first, otherwise verification and reset emails silently fail to send.
</Note>
