...
Built-in providers — Google, GitHub, Apple, Microsoft, GitLab, Discord, and more — are configured per project. Using **GitHub** as the example:

<Steps>
  <Step title="Register an OAuth app with the provider">
    In your provider's developer console (for GitHub, [Developer settings → OAuth Apps](https://github.com/settings/developers)), create a new OAuth app. Set the **Authorization callback URL** to your project's callback endpoint:

    ```
    <project-api-url>/api/auth/oauth/github/callback
    ```

    The provider then issues a **Client ID** and **Client Secret**.
  </Step>
  <Step title="Enable the provider in InsForge">
    In the InsForge dashboard, open **Auth Methods**, enable **GitHub**, and paste the **Client ID** and **Client Secret** from the previous step. If you don't set custom scopes, GitHub defaults to `user:email` so InsForge can read the account's email address.
    {/* TODO: confirm dashboard location — exact per-provider toggle under Auth Methods not verifiable from code */}
  </Step>
  <Step title="Sign users in from your app">
    Call `signInWithOAuth()` with `'github'` as the provider key:
    ...
    The same call works for any enabled provider — swap `'github'` for `'google'`, `'apple'`, and so on. See the [`signInWithOAuth` reference](/sdks/typescript/auth) for the full option list.
  </Step>
</Steps>

<Note>
  Prefer to configure providers programmatically? The admin OAuth config API (`POST /api/auth/oauth/configs`, admin key required) stores the same `provider`, `clientId`, `clientSecret`, and `scopes` fields that the dashboard writes.
</Note>
