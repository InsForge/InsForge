...
<Note>
Verification and password-reset emails are sent through InsForge Cloud by default. To send them from your own server with custom subject lines and HTML, configure [Custom SMTP](/core-concepts/messaging/custom-smtp) under **Authentication → Email** in the dashboard. The four `email-verification-*` and `reset-password-*` templates are fully editable.
</Note>
