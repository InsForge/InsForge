...
Headless clients use an instance-scoped device authorization session and complete login through the existing browser sign-in and consent pages exposed by the current built-in auth app at `/auth/device` and `/auth/device/consent`. This first release does not add `client_id`, `scope`, a registration flow, or multi-instance SSO.
