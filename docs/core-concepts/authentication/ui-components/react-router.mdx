...
<Tip>Want to build custom login pages instead? See [Custom Auth Pages](/core-concepts/authentication/ui-components/customization).</Tip>
...
- `user` - User object with id, email, name, avatarUrl, and other dynamic fields in profile.
- `isLoaded` - Boolean loading state
