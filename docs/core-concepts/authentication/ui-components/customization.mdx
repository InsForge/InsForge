...
| `authConfig.oauthProviders` | `string[]` | Enabled OAuth providers |
...
| `isLoaded` | `boolean` | Whether config has been fetched |
...
### Building Fully Custom UIs

If you need even more control, you can use the SDK methods directly from `useInsforge()`:

```tsx
import { useInsforge, AuthContainer, AuthHeader } from '@insforge/react';

function CompletelyCustomAuth() {
  const { signIn, loginWithOAuth, isLoaded } = useInsforge();
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <button type="button" onClick={() => loginWithOAuth('google', '/dashboard')}>
        Sign in with Google
      </button>
    </form>
  );
...
<Tip>
  For the highest level of control, see the [SDK Reference](/core-concepts/authentication/sdk) to build authentication flows entirely with SDK methods.
</Tip>
