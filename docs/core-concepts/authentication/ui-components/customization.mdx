...
function CustomSignIn() {
  ...
  const availableProviders = [
    ...
    ...(authConfig?.customOAuthProviders ?? []),
  ];

  return (
    <div>
      <p>Available OAuth: {availableProviders.join(', ')}</p>
...
| `authConfig.customOAuthProviders` | `string[]` | Enabled custom OAuth provider keys such as `acme` or `okta-company` |
