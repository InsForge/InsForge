...
**The fastest way** - Use deployed auth pages with one line:
...
const router = createBrowserRouter([
  ...
  // One line adds all auth routes with built-in auth
  ...getInsforgeRoutes({
...
<Note>
With `builtInAuth: false`, `getInsforgeRoutes()` only adds the `/auth/callback` route for OAuth, allowing you to define custom sign-in/sign-up pages.
</Note>
...
- **getInsforgeRoutes()**: React Router helper for built-in auth (similar to Next.js middleware)
