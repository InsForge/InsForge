...
```bash
npm install react@^19.0.0 react-dom@^19.0.0 react-router-dom@^7.9.0
```
</Note>
...
- `afterSignInUrl` (optional): Redirect URL after successful sign in (default: `'/'`)
...
const router = createBrowserRouter([
  {
    ...
    children: [
      ...
      // ✅ Required: Create callback route to handle authentication
...
- Visiting `/reset-password` → Redirects to `your-project.insforge.app/auth/reset-password`
- Visiting `/verify-email` → Redirects to `your-project.insforge.app/auth/verify-email`
...
1. User clicks "Sign In" → Visits /sign-in route
   ↓
2. getInsforgeRoutes() redirects → your-project.insforge.app/auth/sign-in
   ↓
3. User signs in on deployed page → Backend processes authentication
   ↓
4. Backend redirects → yourapp.com/auth/callback?access_token=xxx&user_id=xxx
   ↓
5. <InsforgeCallback /> component:
   ...
   ↓
6. User sees authenticated UI → Dashboard
```
...
Build custom auth flows with pre-built forms. These components are controlled—you manage the state and logic.
