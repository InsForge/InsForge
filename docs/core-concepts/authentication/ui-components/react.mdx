...
createRoot(document.getElementById('root')!).render(
  <StrictMode>
    <InsforgeProvider
      baseUrl={
        import.meta.env.VITE_INSFORGE_BASE_URL || 'https://your-project.insforge.app/' // Replace with your InsForge backend URL
      }
...
- `builtInAuth` (optional): By default use InsForge hosted auth pages
- `paths` (optional): Only use if you customized path names
- `publicRoutes` (optional): Define what routes are open to public
- `loadingFallback` (optional): Custom component to display when redirecting to signin or signup page
...
export default function App() {
  ...
  // Simple path-based routing (or use a simpler router library)
...
- `/sign-in` → Redirects to `https://your-project.insforge.app/auth/sign-in`
...
1. User clicks "Sign In" → Visits /sign-in route
   ↓
2. getInsforgeRoutes() redirects → https://your-project.insforge.app/auth/sign-in?redirect=yourapp.com/you-defined-destination-after-sign-in
   ↓
