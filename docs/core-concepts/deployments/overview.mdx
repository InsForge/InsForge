...
Deploy from your app's source directory. The CLI uploads the source tree, skips local-only files such as `node_modules`, `.git`, build output, and `.env` files, then starts the Vercel build through InsForge. The example uses `./frontend`; replace it with your frontend source directory.
...
Review previous runs, sync Vercel status, inspect metadata, and cancel in-progress deployments from the Deployment Logs page. Replace the example ID with the ID from the deployment list.

```bash
npx @insforge/cli deployments list
npx @insforge/cli deployments status 123e4567-e89b-12d3-a456-426614174000 --sync
npx @insforge/cli deployments cancel 123e4567-e89b-12d3-a456-426614174000
```
...
- Run `npx @insforge/cli deployments deploy ./frontend`, replacing `./frontend` with the frontend source directory.
