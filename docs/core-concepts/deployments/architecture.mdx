...
1. **Create Deployment**: Agent calls `POST /api/deployments/direct` and receives a deployment ID
2. **Create Manifest**: Agent calls `POST /api/deployments/:id/manifest` with each relative file path, SHA-1 digest, and byte size
3. **Upload Files**: Agent streams each file to `PUT /api/deployments/:id/files/:fileId/content`; InsForge forwards it to Vercel with provider credentials kept server-side
...
| Status | Description |
|--------|-------------|
| `WAITING` | Deployment created, awaiting source zip upload or direct manifest/file uploads |
...
- **Lower Memory Use**: Source archives are no longer buffered and unzipped by the backend
