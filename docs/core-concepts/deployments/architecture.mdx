...
3. **Retry If Needed**: If an upload is interrupted, the agent can retry individual file uploads for the same deployment ID. To inspect progress, query `system.deployment_files` with a raw SQL tool for that `deployment_id`
