...
This limit is enforced only when SMTP is enabled. The cloud provider has its own per-plan limits documented in [Architecture](/core-concepts/messaging/architecture#rate-limits).
