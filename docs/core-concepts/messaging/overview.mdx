...
description: "Send transactional emails from your InsForge project with the managed sender or custom SMTP. Phone sign-in codes ship over SMS today; push is on the roadmap for the same API."
---

InsForge Messaging sends transactional notifications from your project: receipts, digests, password-reset codes, notification roll-ups, anything you would otherwise wire SendGrid, Postmark, or Twilio in for. Email is the first channel. SMS is live for phone sign-in codes via [Custom SMS](/core-concepts/messaging/custom-sms); a general SMS send API and push are on the roadmap and will share the same API surface.
