---
title: "Messaging: send transactional email from your project"
sidebarTitle: "Overview"
description: "Send transactional messages from your InsForge project. Covers email today, SMTP setup for self-hosters, and planned SMS and push channels."
---
...
<Warning>
  **Self-hosting?** The managed InsForge Cloud sender is only available to cloud-linked projects. On a self-hosted instance you must configure [Custom SMTP](/core-concepts/messaging/custom-smtp) before any email can send, auth emails included. Without it, turning on email verification leaves new sign-ups stuck in an unverified state with no email ever delivered, because the send failure is logged and swallowed rather than surfaced. Configure SMTP first, or leave email verification off (the default).
</Warning>
