...
description: "Web scraping for your app, powered by Apify."
---
...
Behind the prompt, `npx @insforge/cli webscraper apify login` fetches your InsForge-managed Apify token, logs the local Apify CLI in headlessly (no browser OAuth), and installs the Apify agent skills. From there the agent picks an actor from the Apify Store, starts runs, and reads the results back.
