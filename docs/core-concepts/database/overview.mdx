...
New tables have RLS **on** by default; pass `rlsEnabled: false` at creation to opt out. Requests made with the admin API Key (`ik_...`) bypass RLS. To toggle it on an existing table, run SQL — `ALTER TABLE public.mytable ENABLE ROW LEVEL SECURITY;` (or `DISABLE`) — with `npx @insforge/cli db query` or a migration. Enabling RLS on a table with no policy denies all access to `anon` and `authenticated` until you add one. See the [FAQ](/faq) for policy examples.
