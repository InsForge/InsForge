...
<Info>
InsForge does not require a separate local database stack for migrations. `db migrations up` applies directly to the linked backend.
</Info>
...
InsForge migrations are forward-only today. You usually write the SQL yourself, apply changes in version order, and test destructive changes in a non-production project first.
...
- Migrations are forward-only today. There is no `down` or rollback command in the CLI.
...
### Need a clean-room test environment

The CLI does not currently include a built-in database reset command. Use a disposable InsForge project or a staging backend when you need to rehearse destructive changes safely.

### Need automatic schema diff generation

The CLI does not currently include automatic schema diff generation. Write the SQL by hand, or copy generated SQL from your editor or dashboard workflow into a new migration file created with `db migrations new`.
