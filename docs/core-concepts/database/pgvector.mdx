...
<Note>
  `OPENROUTER_API_KEY` is your own OpenRouter key. On Pro and Enterprise, `npx @insforge/cli ai setup` provisions one and writes it to your env file. Free plans don't include AI model credits, so create a key at [openrouter.ai/keys](https://openrouter.ai/keys) and set it yourself.
</Note>
