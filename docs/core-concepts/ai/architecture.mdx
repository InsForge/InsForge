...
When a BYOK key is active, usage is not tracked by InsForge — billing goes directly to the developer's OpenRouter account. Usage tracking only applies to cloud-managed and environment variable sources.
