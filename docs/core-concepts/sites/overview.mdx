...
### 404: NOT_FOUND on client-side routes

Single-page apps that handle routing in the browser (React Router, Vue Router, and similar) can return `404: NOT_FOUND` when a deep link or a page refresh hits a path that has no matching file on the server. Because InsForge builds your site on Vercel, add a `vercel.json` at your project root that rewrites every path to `index.html`, so the browser router takes over:
...
If the 404 appears on the very first load rather than only on a sub-route, the build output directory is likely wrong for your framework. Vercel detects it from `package.json` and `vercel.json`, so confirm your build produces the expected output folder.
