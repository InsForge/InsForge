...
| **Permission Model** | RLS Policy | Controls subscribe and publish access |
...
The `realtime.messages` table stores all published messages for audit and replay:
...
InsForge Realtime uses PostgreSQL Row Level Security (RLS) to control channel access:
...
TO authenticated
USING (
  realtime.channel_name() LIKE 'order:%'
  AND EXISTS (
...
<Steps>
  ...
  <Step title="Configure Permissions">
    Add RLS policies for subscribe and publish access:
    ```sql
...
<CardGroup cols={2}>
  ...
  <Card title="RLS Security" icon="shield">
    Uses PostgreSQL RLS policies you already know
  </Card>
...
<CardGroup cols={2}>
  ...
  <Card title="Design Policies Carefully" icon="gavel">
    Test RLS policies thoroughly since they control all realtime access
  </Card>
