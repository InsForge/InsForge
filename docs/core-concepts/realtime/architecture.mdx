...
| Feature | Description |
|---------|-------------|
| **Default Policy** | 30 days of retention |
| **Configurable** | Change the `realtime_retention_days` value in the `_config` table |
| **Cleanup Schedule** | Runs daily at midnight via scheduled job runner |
| **Pruning Path** | Native database deletes executed in batches |

### How It Works

1. A background schedule calls the `/api/realtime/messages/cleanup` Admin endpoint daily.
2. The endpoint invokes the `realtime.cleanup_messages()` PostgreSQL function.
3. Rows older than the configured threshold are safely deleted in batches to avoid locking the table or impacting live traffic.

Admins can view the active `retentionDays` setting in the Realtime Messages page statistics dashboard.
