...
Realtime message history is retained automatically so the `realtime.messages` table does not grow without bound.

| Setting | Default | Notes |
|--------|---------|-------|
| **Retention period** | 30 days | Configurable per project by admins |
| **Cleanup cadence** | Every 15 minutes | Scheduled automatically with `pg_cron` |
| **Cleanup batch size** | 1000 rows | Oldest rows are deleted first |
| **Minimum retention** | 1 day | Prevents cleanup from racing fresh delivery updates |

Admins can review and update the retention policy from the Realtime Messages page in the dashboard, or through the admin API at `/api/realtime/messages/config`.

<Note>
Cleanup only prunes historical rows from `realtime.messages`. Channel definitions in `realtime.channels` are left untouched.
</Note>
...
| **Retention** | Automatic | Old rows are pruned in batches after the configured retention period |
