...
description: "Run long-lived containers next to your InsForge project for queue workers, AI inference, websocket servers, or scrapers."
---
...
<Tabs>
  <Tab title="Docker (your own host)">
    ...
    Enabling it is a single deliberate act: mount the Docker socket into the InsForge container. In your compose file, uncomment the two lines that are already there for this:
    ...
    `group_add` is required because the socket is mode `660 root:docker` — without a matching group the backend gets `EACCES`. The group id is host-specific, so put the value your host reports on `.env`'s `DOCKER_GID` line:

    <CodeGroup>
    ```bash Linux
    # 993 on Amazon Linux 2023, commonly 999 on Debian/Ubuntu
    getent group docker | cut -d: -f3
    ```

    ```bash Docker Desktop (macOS/Windows)
    # The socket inside the VM is owned by root:root, so use 0
    echo 0
    ```
    </CodeGroup>
...
Each service picks an ingress mode. The default is `none`, because most compute — queue workers, processors, inference loops — takes no inbound traffic at all.
...
```bash
# 1. Reserve the name (no image yet)
