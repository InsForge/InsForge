...
<Warning>
  Deleting a bucket is permanent. It cascade-deletes every object the bucket
  contains, and there is no trash, soft-delete, or restore — neither self-serve
  nor through support. Download anything you need first, and reserve the delete
  action (`DELETE /api/storage/buckets/{bucketName}`, the `delete-bucket` MCP
  tool, or the SDK) for buckets you are certain you no longer need.
</Warning>
