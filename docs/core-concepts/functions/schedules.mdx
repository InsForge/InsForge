...
Outbound requests use a configurable timeout and do not follow redirects by default. Configure `OUTBOUND_REQUEST_TIMEOUT_MS`, `OUTBOUND_MAX_RESPONSE_BYTES`, and `OUTBOUND_MAX_REDIRECTS` in the backend environment when a deployment needs different limits.
