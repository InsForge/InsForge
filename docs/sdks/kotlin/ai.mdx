...
- `tools` (`List<Tool>`?, optional) - Tool definitions for function calling
- `toolChoice` (ToolChoice?, optional) - Controls tool usage: `ToolChoice.auto()`, `ToolChoice.none()`, `ToolChoice.required()`, or `ToolChoice.function("name")`
...
val response = client.ai.chatCompletion(
    ...
    messages = listOf(
        ChatMessage.user("Your prompt here")
    ),
    tools = listOf(
        Tool(
            type = "function",
            function = FunctionDefinition(
                ...
                parameters = mapOf(
                    ...
                    "properties" to mapOf(
                        "param1" to mapOf(
                            "type" to "string",
                            "description" to "First parameter"
                        )
                    ),
...
- `tools` (`List<Tool>`?, optional) - Tool definitions for function calling
- `toolChoice` (ToolChoice?, optional) - Controls tool usage
- `parallelToolCalls` (Boolean?, optional) - Allow the model to call multiple tools in parallel
