...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `redirect_uri` | string | Yes | URL to redirect after authentication. Must match the redirect URL whitelist when one is configured. |
...
- Redirect URL matching uses exact normalized URLs. `https://app.example.com/callback` and `https://app.example.com/callback/extra` are treated as different URLs.
- If the redirect URL whitelist is empty, InsForge remains permissive for development convenience, but that state is insecure for production and should be avoided.
</Note>
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `redirect` | string | Yes | URL-encoded redirect URI. Must match the redirect URL whitelist when one is configured. |
| `client_type` | string | No | Client type: `web` (default), `mobile`, `desktop`, or `server` |
...
| `redirectUrlWhitelist` | string[] | No | Exact trusted redirect URLs. If configured, `signInRedirectTo`, OAuth `redirect_uri`, and email verification redirects must match one of these URLs. |
