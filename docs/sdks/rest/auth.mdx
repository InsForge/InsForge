...
Headless clients can start a short-lived device authorization session and complete it through the existing browser sign-in and consent pages at `/auth/device` and `/auth/device/consent`. This release is instance-scoped only: it does not support `client_id`, `scope`, a registration flow, or multi-instance SSO.
