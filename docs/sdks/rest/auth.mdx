...
Headless clients can start a short-lived device authorization session and complete it through the current built-in auth app's device-code entry and consent pages at `/auth/device` and `/auth/device/consent`. If the browser is not already signed in, the flow continues through the existing `/auth/sign-in` route before returning to consent. This release is instance-scoped only: it does not support `client_id`, `scope`, a registration flow, or multi-instance SSO.
...
| `hostname` | string | No | Hostname reported by the device |
| `platform` | string | No | Platform string reported by the device |

### Example

```bash
curl -X POST "https://your-app.insforge.app/api/auth/device/authorizations" \
  -H "Content-Type: application/json" \
  -d '{
    "deviceName": "my-vps",
    "hostname": "vps-01",
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `userCode` | string | Yes | Code entered by the user in the browser |

### Example

```bash
curl -X POST "https://your-app.insforge.app/api/auth/device/authorizations/lookup" \
  -H "Content-Type: application/json" \
  -d '{
    "userCode": "ABCDE-FGHIJ"
  }'
...
<Note>
The public lookup response exposes only `status` and `expiresAt`. When the browser is already authenticated, the same endpoint also binds the device authorization to that signed-in user and includes `clientContext` for the consent page.
</Note>
...
This endpoint requires an authenticated browser session. It returns the minimal lookup view after approval.
...
This endpoint requires an authenticated browser session. It returns the minimal lookup view after denial.

### Exchange Device Authorization
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `deviceCode` | string | Yes | Device code returned from create |
| `grantType` | string | Yes | Must be `urn:insforge:params:oauth:grant-type:device_code` |

### Example

```bash
curl -X POST "https://your-app.insforge.app/api/auth/device/token" \
  -H "Content-Type: application/json" \
  -d '{
    "deviceCode": "device-code-123",
...
| Endpoint | Error | HTTP | Meaning |
|----------|-------|------|---------|
| `POST /api/auth/device/authorizations/lookup` | `NOT_FOUND` | 404 | The `userCode` does not match an active authorization session |
| `POST /api/auth/device/token` | `authorization_pending` | 428 | The authorization has not been approved yet |
| `POST /api/auth/device/token` | `slow_down` | 429 | The client is polling too quickly |
| `POST /api/auth/device/token` | `access_denied` | 403 | The user denied the request |
| `POST /api/auth/device/token` | `expired_token` | 400 | The authorization expired before exchange |
| `POST /api/auth/device/token` | `already_used` | 400 | The authorization was already exchanged |
| `POST /api/auth/device/authorizations/approve` | `AUTH_UNAUTHORIZED` | 401 | The browser session is not signed in |
