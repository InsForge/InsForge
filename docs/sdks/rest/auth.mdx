...
| `verifyEmailUrl` | string | No | Used for link-based email verification. The email link opens this URL with the verification `token` in the query string so your app can verify the email itself. Required when `verifyEmailMethod` is `link`. |
...
curl -X POST "https://your-app.insforge.app/api/auth/users?client_type=mobile" \
  ...
  -d '{
    ...
    "password": "securepassword123",
    "name": "John Doe",
    "verifyEmailUrl": "myapp://verify-email"
  }'
```
...
|-------|------|----------|-------------|
| `email` | string | Yes | User email address |
| `resetPasswordUrl` | string | No | Used for link-based password reset. The magic link opens this URL with the reset `token` in the query string so your app can render its own reset-password page. Required when `resetPasswordMethod` is `link`. |
...
```json
{
  ...
  "verifyEmailMethod": "code",
  "resetPasswordMethod": "link",
  "redirectUrlWhitelist": ["https://myapp.com/dashboard", "https://*.myapp.com"],
  "createdAt": "2024-01-15T10:30:00Z",
  "updatedAt": "2024-01-15T10:30:00Z"
}
```
...
| `redirectUrlWhitelist` | array | No | List of allowed redirect URLs (supports wildcards like `*.domain.com`) |
