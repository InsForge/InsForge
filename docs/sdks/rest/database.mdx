...
Create and immediately execute a custom migration against the `public` schema. The migration is recorded only if every statement succeeds. **Requires admin authentication.**
...
|-------|------|----------|-------------|
| `name` | string | Yes | Unique migration name |
...
<Warning>
Custom migrations may only target the `public` schema. Statements that reference internal schemas such as `auth`, `system`, `storage`, `ai`, `functions`, `realtime`, or `schedules`, or that attempt to manage `search_path` or their own transactions, are rejected.
</Warning>
