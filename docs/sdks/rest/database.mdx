...
### Example: change a value in a table

`INSERT`, `UPDATE`, and `DELETE` run here too — this is one way to change a value directly in a table from the REST API when the [Update Records](#update-records) endpoint isn't enough. System tables and `auth.users` stay blocked, so this works on your own tables in `public`. For schema changes, prefer a [migration](#create-database-migration) instead.
...
The response `command` field reflects the statement type (for example `UPDATE`) and `rowCount` is the number of rows changed.
