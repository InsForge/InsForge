...
|-------|------|----------|-------------|
| `filename` | string | Yes | Object key to use verbatim unless `autoKey` is enabled |
| `contentType` | string | No | MIME type of the file |
| `size` | integer | No | File size in bytes |
| `upsert` | boolean | No | Replace an existing object at this key. Defaults to `false` |
| `autoKey` | boolean | No | Generate a unique key from `filename`. Defaults to `false` |
...
Confirm that a file was successfully uploaded to S3 using a presigned URL. Call the exact `confirmUrl` returned by the upload strategy; for S3 it contains the server-owned staging identifier.
...
| `upsert` | boolean | No | Must match the `upsert` value used to request the strategy |
