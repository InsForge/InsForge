...
```
GET /api/storage/buckets/{bucketName}/objects/{objectKey}/download-strategy
```

<Note>
  Expiry (for presigned URLs) is auto-calculated server-side from bucket
  visibility. The endpoint takes no request body.

  `POST` on the same path is retained as a deprecated alias for older SDK
...
```bash
curl "https://your-app.insforge.app/api/storage/buckets/avatars/objects/profile.jpg/download-strategy" \
  -H "Authorization: Bearer your-jwt-token"
```
