...
- `toolChoice` (ToolChoice?, optional) - Controls tool usage: `.auto`, `.none`, `.required`, or `.function("name")`
- `parallelToolCalls` (Bool?, optional) - Allow the model to call multiple tools in parallel
...
public struct FunctionDefinition: Codable, Sendable {
    ...
    let parameters: AnyCodable?         // JSON Schema (use AnyCodable from open-source package)
}
...
public enum ToolChoice: Codable, Sendable {
    ...
    case none
    case required
    case function(String)               // Specific function name
}
