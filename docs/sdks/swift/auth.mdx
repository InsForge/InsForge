...
The SDK uses `ASWebAuthenticationSession` to present a secure in-app authentication browser. This provides a seamless experience where authentication completes automatically.

```
1. App calls signInWithDefaultView(redirectTo:)
   ↓
2. SDK opens in-app authentication browser (ASWebAuthenticationSession)
   ↓
3. User authenticates (OAuth or email+password)
   ↓
4. SDK automatically handles callback and creates session
   ↓
5. Method returns AuthResponse with user data
```
    ...
    var body: some Scene {
        WindowGroup {
            ContentView()
                ...
                .onOpenURL { url in
                    // Only needed for external browser fallback mode
                    // ASWebAuthenticationSession handles callbacks automatically
                    Task {
                        do {
...
struct ContentView: View {
    ...
    var body: some View {
        if let user = currentUser {
            ...
            OAuthLoginView(onAuthenticated: { user in
                currentUser = user
            })
        }
    }
...
struct OAuthLoginView: View {
    ...
    func OAuthButton(provider: OAuthProvider, title: String, icon: String) -> some View {
        Button {
            Task {
                do {
                    // ASWebAuthenticationSession returns AuthResponse directly
                    if let response = try await client.auth.signInWithOAuthView(
                        provider: provider,
                        redirectTo: "yourapp://auth/callback"
                    ) {
                        // Authentication completed via in-app browser
                        onAuthenticated(response.user)
                    }
                    // If nil, external browser was used - callback handled via onOpenURL

                    // For production with Universal Links:
                    // if let response = try await client.auth.signInWithOAuthView(
                    //     provider: provider,
                    //     redirectTo: "https://yourdomain.com/auth/callback"
                    // ) {
...
- ✅ Use **ASWebAuthenticationSession** (default on iOS 12+/macOS 10.15+) for secure in-app authentication
- ✅ Use **Universal Links** (iOS) or **App Links** (Android) for callback URLs in production
...
The SDK uses `ASWebAuthenticationSession` which provides:

| Feature | Benefit |
|---------|---------|
| **Secure Browser** | Isolated from app's WebView, cookies shared with Safari |
| **Automatic Callback** | No need for URL scheme hijacking concerns |
| **Session Isolation** | Authentication data protected from the app |
| **User Trust** | Shows Apple's authentication sheet UI |
| **PKCE Support** | Enhanced OAuth security built-in |

#### Why Universal Links are Recommended for Production

1. **Domain Verification**: Apple verifies you own the domain by checking the hosted configuration file
...
4. **Better UX**: Seamless experience without redirect prompts
