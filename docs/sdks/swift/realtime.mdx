...
Events are published by database triggers that call `realtime.publish(...)` or by clients that publish to a channel they have already joined. See [Realtime Architecture](/core-concepts/realtime/architecture) for channel matching and RLS details.
