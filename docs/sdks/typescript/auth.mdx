...
- `options` (object, optional) - Additional options
  - `emailRedirectTo` (string, optional) - Custom URL to redirect to after email verification. It's a fallback if you didn't set the default redirect URL in your dashboard, and it must match the redirect URL whitelist when one is configured.
...
<Note>
  Redirect destinations are checked against the Auth Settings redirect URL whitelist when one is configured. Matching uses exact normalized URLs, not prefix matching. If the whitelist is empty, InsForge stays permissive for development convenience, but that state is insecure for production.
</Note>
...
- `redirectTo` (string, optional) - URL to redirect after authentication. It must match the redirect URL whitelist when one is configured.
...
<Note>
  OAuth redirect matching uses exact normalized URLs, not prefix matching. If the redirect URL whitelist is empty, InsForge allows redirects for development convenience, but that state is insecure for production.
</Note>
