...
- `extraParams` (Record<string, string>, optional) - Additional OAuth parameters to include in the authorization URL (e.g., `{ prompt: 'select_account' }`, `{ access_type: 'offline' }`)
...
```javascript
// With extraParams: force account selection for Google
