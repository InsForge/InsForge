...
const checkout = new RazorpayCheckout({
  key: orderResponse.checkoutOptions.keyId,
  amount: orderResponse.checkoutOptions.amount,
  currency: orderResponse.checkoutOptions.currency,
  order_id: orderResponse.checkoutOptions.orderId,
  description: orderResponse.checkoutOptions.description ?? undefined,
  prefill: orderResponse.checkoutOptions.prefill,
...
const checkout = new RazorpayCheckout({
  key: subscriptionResponse.checkoutOptions.keyId,
  subscription_id: subscriptionResponse.checkoutOptions.subscriptionId,
  description: subscriptionResponse.checkoutOptions.description ?? undefined,
  prefill: subscriptionResponse.checkoutOptions.prefill,
  handler: async (response: {
...
Subscription creation checks `INSERT` policies on `payments.razorpay_subscriptions`. Cancel, pause, and resume check `UPDATE` policies on the same table. Add app-specific RLS or server-side membership checks before exposing these controls for shared subjects.

## Fulfillment

Do not mark orders paid, grant credits, or activate subscriptions from the Checkout handler alone. Use verified Razorpay webhook events in `payments.webhook_events`.
...
<Warning>
  ...
  only receives the Razorpay Key ID inside `checkoutOptions`.
</Warning>
