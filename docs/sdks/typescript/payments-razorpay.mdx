...
InsForge copies `metadata` to Razorpay `notes`. If your webhook trigger reads `notes.order_id`, pass `metadata: { order_id: ... }` when creating the Order.
...
InsForge copies `metadata` to Razorpay subscription `notes`, which webhook triggers can read later.
