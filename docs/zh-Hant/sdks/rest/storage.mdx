---
title: 存儲API參考
description: 透過REST API的檔案存儲和貯體管理
---
...
| 欄位 | 類型 | 必需 | 說明 |
|-------|------|----------|-------------|
| `filename` | string | 是 | 物件金鑰；除非啟用 `autoKey`，否則依原樣使用 |
| `contentType` | string | 否 | 檔案的MIME類型 |
| `size` | integer | 否 | 檔案大小（位元組） |
| `upsert` | boolean | 否 | 取代此金鑰的現有物件。預設為 `false` |
| `autoKey` | boolean | 否 | 根據 `filename` 產生唯一金鑰。預設為 `false` |
...
確認檔案已成功使用預簽名 URL 上傳到 S3。請使用上傳策略傳回的完整 `confirmUrl`；對於 S3，該 URL 包含由伺服器管理的暫存識別碼。
...
| `upsert` | boolean | 否 | 必須與請求上傳策略時使用的 `upsert` 值一致 |
