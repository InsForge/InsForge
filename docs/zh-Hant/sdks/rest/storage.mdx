---
title: 存儲API參考
description: "InsForge 儲存 REST API 完整參考：說明以上傳與下載策略搭配預簽名 URL 存取物件的逐步流程，支援本機與 S3 相容的儲存後端，並涵蓋貯體管理端點、必要驗證標頭、已淘汰的直接上傳與下載介面，以及各端點的請求與回應範例，協助任何後端語言以 HTTP 管理檔案物件。"
---
