---
title: 儲存空間 SDK 參考
description: 使用 InsForge TypeScript SDK 進行檔案上傳、下載和管理
---
...
- `options.upsert` (boolean, 選填) - 取代現有物件。預設為 `false`
...
<Warning>
如果已存在相同金鑰的檔案，`upload()` 預設會傳回衝突。傳入 `{ upsert: true }` 可取代該檔案；若要由伺服器產生唯一金鑰，請使用 `uploadAuto()`。
</Warning>
