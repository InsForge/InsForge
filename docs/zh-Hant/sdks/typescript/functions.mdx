---
title: "TypeScript 函式 SDK 參考"
description: "InsForge TypeScript 函式 SDK 參考：使用 invoke() 以 GET、POST 請求與自訂標頭呼叫無伺服器 Edge Function，並提供公開函式與需身分驗證函式的完整無伺服器範例，示範存取使用者資料、參數傳遞與型別化回應處理方式。"
---
