...
description: "建立 InsForge 後端的隔離分支，各分支擁有獨立的 Postgres、驗證設定、儲存與排程，以 full 或 schema-only 模式建立，透過 CLI 預覽合併 SQL、重設或刪除，安全演練危險的結構描述變更與 RLS 重寫後再合併回父專案。InsForge OSS 2.1.0 起提供。"
---
