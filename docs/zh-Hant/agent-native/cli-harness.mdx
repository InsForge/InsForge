...
| 專案 | `create`, `link`, `projects update`, `projects update-version`, `projects delete` |
...
`projects update` 就地變更專案設定。傳入 `--name "<新名稱>"` 可重新命名專案；名稱至少為 2 個字元，且在您的組織內必須唯一。它預設針對已連結的專案，因此傳入 `--project <id>` 可重新命名其他專案。該命令在底層會發出 `PUT /projects/v1/{projectId}` 請求，請求主體為 `{ "name": "<新名稱>" }`。

您也可以在儀表板的 **Settings → General** 下重新命名專案。
