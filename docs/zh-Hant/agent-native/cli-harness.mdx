---
title: "CLI 工具：代理操作後端的終端介面"
sidebarTitle: "CLI harness"
description: "@insforge/cli 是編碼代理操作 InsForge 後端的統一終端介面：登入與連結專案、建立與套用資料庫遷移、config 計畫與套用、後端分支、部署、專案升級與刪除以及診斷，全面支援 --json 結構化輸出與 --yes 免確認自動化執行。任何終端機或 CI 皆可執行。"
---
