...
description: "InsForge 驗證支援電子郵件密碼、魔法連結、一次性代碼、Google 與 GitHub 等 OAuth 及任何 OIDC 相容提供商，登入時簽發 JWT 供平台所有產品共用，並結合 Postgres 資料列層級安全（RLS）原則落實細緻的授權控管。使用者集中於 auth.users 管理。"
---
