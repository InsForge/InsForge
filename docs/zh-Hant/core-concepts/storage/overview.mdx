...
<Warning>
  刪除儲存貯體是永久性操作。它會級聯刪除儲存貯體中的每一個物件，並且沒有垃圾桶、軟刪除或還原機制——無論是自助方式還是透過技術支援都無法還原。請先下載所有需要的檔案，僅對確定不再需要的儲存貯體執行刪除操作（`DELETE /api/storage/buckets/{bucketName}`、`delete-bucket` MCP 工具或 SDK）。
</Warning>
