...
新資料表預設**開啟** RLS；在建立時傳 `rlsEnabled: false` 可以關閉。使用管理 API Key（`ik_...`）發出的請求會繞過 RLS。要為既有的資料表切換 RLS，用 `npx @insforge/cli db query` 或遷移執行 SQL——`ALTER TABLE public.mytable ENABLE ROW LEVEL SECURITY;`（或 `DISABLE`）。在沒有任何原則的資料表上啟用 RLS，會拒絕 `anon` 和 `authenticated` 的所有存取，直到你新增一條原則。原則範例見 [FAQ](/faq)。
