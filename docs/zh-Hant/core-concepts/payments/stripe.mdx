...
Dashboard 和 CLI 都呼叫同一個管理端點。你也可以用管理 API Key（`ik_...`）直接呼叫它：
...
- `PUT /api/payments/stripe/{environment}/config`，其中 `{environment}` 為 `test` 或 `live`。僅限管理員。
- 請求主體為 `{ "secretKey": "..." }`。密鑰前綴必須與環境相符（`test` 用 `sk_test_...`，`live` 用 `sk_live_...`）。
- 成功後，InsForge 會對照你的 Stripe 帳戶驗證密鑰、加密儲存到祕密儲存、嘗試（重新）建立託管的 webhook 端點，並為 Products、Prices、Customers 和 Subscriptions 執行首次同步。如果連接的 Stripe 帳戶發生變化，會先清除先前鏡像的支付資料。

要讀取連接狀態和遮罩後的密鑰，呼叫 `GET /api/payments/stripe/status`。
