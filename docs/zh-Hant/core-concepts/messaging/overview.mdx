...
description: "透過 InsForge 託管寄件服務或自訂 SMTP 寄送交易性電子郵件；手機登入驗證碼現已透過簡訊傳送，推送通知將共用相同 API 介面。"
---

InsForge Messaging 從您的專案傳送交易性通知：收據、摘要、密碼重設代碼、通知匯總、任何您本來會為其接入 SendGrid、Postmark 或 Twilio 的東西。電子郵件是第一個通道。簡訊已可用於手機登入驗證碼（參見[自訂簡訊](/core-concepts/messaging/custom-sms)）；通用簡訊傳送 API 和推送在路線圖上，將共享相同的 API 表面。
