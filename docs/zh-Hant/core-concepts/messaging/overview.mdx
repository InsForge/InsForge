---
title: "訊息傳遞：從你的 InsForge 專案傳送交易性郵件"
sidebarTitle: "Overview"
description: "從 InsForge 專案傳送交易性訊息，涵蓋目前的電子郵件、自架環境下的 Custom SMTP 設定，以及路線圖上的簡訊與推播通道。"
---
...
<Warning>
  **自託管？** 託管的 InsForge Cloud 寄件服務僅對已連接雲端的專案可用。在自託管實例上，必須先設定 [Custom SMTP](/core-concepts/messaging/custom-smtp)，任何電子郵件才能發出，包括驗證郵件。否則，一旦開啟電子郵件驗證，新註冊使用者會卡在未驗證狀態且永遠收不到郵件，因為傳送失敗被記錄並吞掉，而不會向外顯示。請先設定 SMTP，或將電子郵件驗證保持關閉（預設值）。
</Warning>
