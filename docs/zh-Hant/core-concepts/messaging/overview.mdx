...
<Note>
  **按需郵件需要付費計畫。** 使用 `emails.send()` 傳送交易型訊息僅在付費計畫中可用——在 **Free** 計畫上，該呼叫會返回 `On-demand email service is only available for paid plans`，且不會傳送任何郵件。驗證郵件（驗證碼、密碼重設、一次性驗證碼）是例外：它們在所有計畫上都會傳送，僅受每月郵件用量限制。

  **每個收件者單獨計數。** `to`、`cc` 和 `bcc` 中的每個地址都會作為一次傳送計入速率限制和每月用量——一封傳送給三個收件者的郵件即為三次傳送。付費計畫採用固定的一小時速率限制視窗；每小時的具體額度取決於您的計畫。使用 `npx @insforge/cli usage` 查看目前計畫，並參閱 [定價](/pricing) 了解各計畫包含的內容。
</Note>
