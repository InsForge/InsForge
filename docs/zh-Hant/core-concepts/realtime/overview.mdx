...
description: "用戶端訂閱 InsForge Realtime 頻道即可經 WebSocket 接收資料庫變更、廣播訊息與線上狀態（presence），頻道亦可把相同訊息分發至 webhook URL 供下游服務接收；控制台提供頻道模式、訊息歷史、權限與保留設定管理。訂閱與發布皆經 RLS 檢查。"
---
