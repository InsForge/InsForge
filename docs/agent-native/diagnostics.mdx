...
### Health finding: high dead tuple ratio

The `health/dead-tuples` finding flags a table where too many rows are dead tuples. It reflects standard PostgreSQL behavior, not heavy traffic, so it can fire on a small or lightly used project:

- Every `UPDATE` or `DELETE` leaves the previous row version behind as a *dead tuple*. That is how Postgres MVCC keeps concurrent reads consistent, and it happens on every table that changes.
- Autovacuum reclaims that space, but by default it only runs once dead rows reach 20% of a table. On a small or low-churn table, the ratio can sit above the advisor's threshold for a while before autovacuum catches up.

The remediation reclaims the current dead tuples and makes autovacuum trigger sooner on that table:

```sql
-- reclaim the dead tuples now
VACUUM ANALYZE files;

-- make autovacuum run sooner on this high-churn table
...
Apply the `ALTER TABLE` part as a [migration](/core-concepts/database/migrations) so the tuning is versioned, then re-scan with `diagnose advisor` to confirm the finding cleared.
