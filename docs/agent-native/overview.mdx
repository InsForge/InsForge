...
2. Branch the backend, write a [migration](/core-concepts/database/migrations) or edit `insforge.toml`, then run `npx @insforge/cli config plan` to see the diff.
3. Apply it with `npx @insforge/cli db migrations up` or `config apply`, against the branch first, then the parent.
