...
| Project | `create`, `link`, `projects update`, `projects update-version`, `projects delete` |
...
```bash
npx @insforge/cli projects update --name "My New Name"
```

`projects update` changes project settings in place. Pass `--name "<new name>"` to rename the project; the name must be at least 2 characters and unique within your organization. It targets the linked project by default, so pass `--project <id>` to rename another. Under the hood the command issues `PUT /projects/v1/{projectId}` with `{ "name": "<new name>" }`.

You can also rename a project from the dashboard under **Settings → General**.
