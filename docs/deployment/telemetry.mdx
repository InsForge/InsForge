...
InsForge sends a `instance_started` event when the backend starts and a `heartbeat` event about once every 24 hours while the backend is running.

Each event contains:

- Anonymous installation ID stored locally in `LOGS_DIR/.insforge-installation-id`
- InsForge version
- Event timestamp
...
```bash
INSFORGE_TELEMETRY_ENDPOINT=https://telemetry.example.com/v1/events
```

The endpoint receives JSON `POST` requests. Network failures are logged and ignored so telemetry never blocks startup or normal requests.
